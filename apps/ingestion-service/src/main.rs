...
async fn ingest_handler(
    ...
    let _: () = con.rpush("documents:queue", payload).await
        .map_err(|e| shared_lib::AppError::Redis(e.to_string()))?;
        
    tracing::info!("Document {} pushed to queue", msg.id);
    
    Ok((StatusCode::ACCEPTED, Json(serde_json::json!({ "status": "queued", "id": msg.id }))))
}
...
async fn main() -> Result<()> {
    ...
    let redis_uri = std::env::var("REDIS_URI").unwrap_or_else(|_| "redis://127.0.0.1:6379/".to_string());
